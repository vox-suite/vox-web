...
test("plugin catalog defines required consumer apps with unique IDs and valid capabilities", () => {
  ...
  for (const plugin of PLUGIN_CATALOG) {
    ...
    assert.ok(plugin.description.length > 0, `Description missing for ${plugin.id}`);
    assert.ok(plugin.logoFile.endsWith(".svg"), `Logo must be an svg file: ${plugin.id}`);
    assert.ok(plugin.backgroundColor.length > 0, `Background color missing for ${plugin.id}`);
    assert.ok(typeof plugin.isPopular === "boolean", `isPopular must be a boolean: ${plugin.id}`);
    assert.ok(["oauth2", "api_key", "none"].includes(plugin.authType), `Invalid authType: ${plugin.authType}`);
    assert.ok(plugin.publisher.length > 0, `Publisher missing for ${plugin.id}`);
    assert.ok(plugin.rating >= 1 && plugin.rating <= 5, `Rating must be between 1 and 5: ${plugin.rating}`);
    assert.ok(plugin.installsCount >= 0, `Installs count must be non-negative: ${plugin.installsCount}`);
    ...
    for (const cap of plugin.capabilities) {
      ...
      assert.ok(cap.description.length > 0, `Capability description missing in ${plugin.id}`);
      assert.ok(
  ...
  for (const requiredId of REQUIRED_APP_IDS) {
    assert.ok(ids.has(requiredId), `Missing required app in catalog: ${requiredId}`);
  }
  ...
  assert.equal(getCatalogPlugin("nonexistent"), undefined, "Non-existent plugin should return undefined");
  ...
  assert.ok(categories["Rides & Travel"].length > 0, "Rides & Travel must not be empty");
  assert.ok(categories["Lifestyle & Essentials"].length > 0, "Lifestyle & Essentials must not be empty");
});
...
test("all referenced logo files exist in public/plugins/logos", () => {
  ...
  for (const plugin of PLUGIN_CATALOG) {
    ...
    assert.ok(fs.existsSync(filePath), `Logo file does not exist on disk: ${plugin.logoFile}`);
  }
});
