...
export function GrantsManager() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (!grantsRes.ok || !connRes.ok)
        throw new Error("Failed to load grants or connections");
  ...
  async function handleRevoke(connectionId: string, capability: string) {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to revoke capability",
      );
    }
  }

  const authorizedConnections = connections.filter(
    (c) => c.authorization_state === "authorized",
  );
      ...
      >
        <Stack gap="normal">
          <div className="p-3 bg-obsidian border border-border-edge rounded-lg text-sm text-mist">
            <strong>Default-Deny Principle:</strong> New agents receive{" "}
            <strong>zero capability grants by default</strong>. Connecting an
            account does not authorize agents to act. You must explicitly grant
          ...
          {!loading && (
            <div className="space-y-4">
              ...
              {grants.length === 0 ? (
                <Text muted>
                  No active capability grants for this agent. The agent cannot
                ...
                <div className="space-y-2">
                  {grants.map((grant) => (
                    <div
                      key={grant.id}
                      className="p-3 border border-border-edge bg-ink rounded flex items-center justify-between"
                      data-testid={`grant-${grant.id}`}
                    >
                      <div>
                        <strong className="text-sm text-mist">
              ...
              <h4 className="text-sm font-semibold text-mist pt-3">
                Connected Services & Capabilities Matrix
              </h4>

              {authorizedConnections.length === 0 ? (
                <Text muted>
                  No authorized connections available. Connect an account under
                ...
                <div className="space-y-3">
                  {authorizedConnections.map((conn) => (
                    <div
                      key={conn.id}
                      className="p-3 border border-border-edge rounded bg-obsidian/40 space-y-2"
                    >
                      <Row spread>
                        <span className="font-medium text-sm text-mist">
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        {conn.authorized_capabilities.map((cap) => {
                          const hasGrant = grants.some(
                            (g) =>
                              g.connection_id === conn.id &&
                              g.capability_external_key === cap,
                          );
                          return (
                            <div
                              key={cap}
                              className="flex items-center gap-2 p-2 bg-ink rounded border border-border-edge text-xs"
                            >
                              <span>{cap}</span>
                              {hasGrant ? (
