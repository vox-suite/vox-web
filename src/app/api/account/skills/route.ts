...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Unable to load skills" }, { status: 502 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  if (!skill.external_key || !skill.title || !skill.summary || !skill.instructions) {
    return NextResponse.json({ error: "Complete all required skill fields" }, { status: 400 });
  }
  try {
    const created = await core.publishPrivateSkill(account.accountId, skill);
    return NextResponse.json({ skill: created }, { status: 201 });
  } catch {
    return NextResponse.json({ error: "Unable to save skill" }, { status: 502 });
  }
}
